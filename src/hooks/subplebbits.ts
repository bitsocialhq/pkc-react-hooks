...
const log = Logger("bitsocial-react-hooks:subplebbits:hooks");
  ...
  useEffect(() => {
    ...
    if (!subplebbit && !onlyIfCached) {
      ...
      addSubplebbitToStore(subplebbitAddress, account).catch((error: unknown) =>
        log.error("useSubplebbit addSubplebbitToStore error", { subplebbitAddress, error }),
      );
    }
...
export function useSubplebbitStats(options?: UseSubplebbitStatsOptions): UseSubplebbitStatsResult {
  ...
  const subplebbitStats = useSubplebbitsStatsStore(
    (state: SubplebbitsStatsState) => state.subplebbitsStats[subplebbitAddress || ""],
  );
  const setSubplebbitStats = useSubplebbitsStatsStore(
    (state: SubplebbitsStatsState) => state.setSubplebbitStats,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        log.error("useSubplebbitStats plebbit.fetchCid error", {
          subplebbitAddress,
          subplebbitStatsCid,
          subplebbit,
          fetchedCid,
...
const useSubplebbitsStatsStore = createStore<SubplebbitsStatsState>((setState: Function) => ({
  ...
  setSubplebbitStats: (subplebbitAddress: string, subplebbitStats: SubplebbitStats) =>
    setState((state: SubplebbitsStatsState) => ({
...
export function useResolvedSubplebbitAddress(
  options?: UseResolvedSubplebbitAddressOptions,
): UseResolvedSubplebbitAddressResult {
  assert(
    !options || typeof options === "object",
    ...
    () => {
      ...
      (async () => {
        try {
          ...
          setState("failed");
          setResolvedAddress(undefined);
          log.error("useResolvedSubplebbitAddress resolveSubplebbitAddress error", {
            subplebbitAddress,
            chainProviders,
            error,
          });
...
export const resolveSubplebbitAddress = async (
