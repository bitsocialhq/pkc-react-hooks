...
describe("subplebbits", () => {
  ...
  describe("no subplebbits in database", () => {
    ...
    test("get subplebbits one at a time", async () => {
      ...
      Subplebbit.prototype.simulateUpdateEvent = () => {
        if (throwOnSubplebbitUpdateEvent) {
          throw Error(
            "no subplebbit update events should be emitted when subplebbit already in store",
          );
        }
      };
    ...
    test("get multiple subplebbits at once", async () => {
      const rendered = renderHook<any, any>((subplebbitAddresses) =>
        useSubplebbits({ subplebbitAddresses }),
      );
      const waitFor = testUtils.createWaitFor(rendered);
      ...
      expect(rendered.result.current.subplebbits[0].description).toBe(
        "subplebbit address 1 description updated",
      );
      expect(rendered.result.current.subplebbits[1].description).toBe(
        "subplebbit address 2 description updated",
      );
      expect(rendered.result.current.subplebbits[2].description).toBe(
        "subplebbit address 3 description updated",
      );
    });
    ...
    test("plebbit.createSubplebbit throws adds useSubplebbit().error", async () => {
      ...
      await waitFor(
        () => rendered.result.current.error.message === "plebbit.createSubplebbit error",
      );
  ...
  describe("useResolvedSubplebbitAddress", () => {
    ...
    test.skip("useResolvedSubplebbitAddress", { timeout }, async () => {
      const rendered = renderHook<any, any>((subplebbitAddress) =>
        useResolvedSubplebbitAddress({ subplebbitAddress }),
      );
