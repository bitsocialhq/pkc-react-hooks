...
const subplebbitsPagesStore = createStore<SubplebbitsPagesState>(
  (setState: Function, getState: Function) => ({
    ...
    addNextSubplebbitPageToStore: async (
      ...
      assert(
        ...
        `subplebbitsPagesStore.addNextSubplebbitPageToStore subplebbit '${subplebbit}' invalid`,
      );
      assert(
        sortType && typeof sortType === "string",
        `subplebbitsPagesStore.addNextSubplebbitPageToStore sortType '${sortType}' invalid`,
      );
      assert(
        typeof account?.plebbit?.createSubplebbit === "function",
        `subplebbitsPagesStore.addNextSubplebbitPageToStore account '${account}' invalid`,
      );
      assert(
        !modQueue || Array.isArray(modQueue),
        `subplebbitsPagesStore.addNextSubplebbitPageToStore modQueue '${modQueue}' invalid`,
      );
      ...
      if (!subplebbitFirstPageCid) {
        log(
          `subplebbitsPagesStore.addNextSubplebbitPageToStore subplebbit '${subplebbit?.address}' sortType '${sortType}' no subplebbitFirstPageCid`,
        );
        return;
      }
...
const onSubplebbitPostsClientsStateChange =
  ...
  (clientState: string, clientType: string, sortType: string, clientUrl: string) => {
    subplebbitsStore.setState((state: SubplebbitsState) => {
      ...
      subplebbit.posts.clients[clientType][sortType] = {
        ...subplebbit.posts.clients[clientType][sortType],
      };
...
const fetchPage = async (
  ...
  if (!fetchPageSubplebbits[subplebbitAddress]) {
    ...
    utils.pageClientsOnStateChange(
      fetchPageSubplebbits[subplebbitAddress][pageType]?.clients,
      onSubplebbitPostsClientsStateChange(subplebbitAddress),
    );
  }

  const onError = (error: any) =>
    log.error(
      `subplebbitsPagesStore subplebbit '${subplebbitAddress}' failed subplebbit.posts.getPage page cid '${pageCid}':`,
      error,
    );
  const fetchedSubplebbitPage = await utils.retryInfinity(
    () => fetchPageSubplebbits[subplebbitAddress][pageType].getPage({ cid: pageCid }),
    { onError },
  );
 ...
 */
export const getSubplebbitPages = (
  subplebbit: Subplebbit,
  sortType: string,
  subplebbitsPages: SubplebbitsPages,
  pageType: string,
  ...
  assert(
    subplebbitsPages && typeof subplebbitsPages === "object",
    `getSubplebbitPages subplebbitsPages '${subplebbitsPages}' invalid`,
  );
...
export const getSubplebbitFirstPageCid = (
  subplebbit: Subplebbit,
  sortType: string,
  pageType = "posts",
) => {
  ...
  assert(
    sortType && typeof sortType === "string",
    `getSubplebbitFirstPageCid sortType '${sortType}' invalid`,
  );
