...
const subplebbitsDatabase = localForageLru.createInstance({
  name: "plebbitReactHooks-subplebbits",
  size: 500,
});
import Logger from "@plebbit/plebbit-logger";
const log = Logger("bitsocial-react-hooks:subplebbits:stores");
...
const subplebbitsStore = createStore<SubplebbitsState>(
  (setState: Function, getState: Function) => ({
    ...
    async addSubplebbitToStore(subplebbitAddress: string, account: Account) {
      ...
      assert(
        ...
        `subplebbitsStore.addSubplebbitToStore invalid account argument '${account}'`,
      );
      ...
      if (!subplebbit) {
        ...
        if (subplebbitData) {
          ...
          try {
            ...
            console.error("failed plebbit.createSubplebbit(cachedSubplebbit)", {
              cachedSubplebbit: subplebbitData,
              error: e,
            });
          }
        }
      ...
      if (!subplebbit) {
        ...
        throw (
          errorGettingSubplebbit ||
          Error(
            `subplebbitsStore.addSubplebbitToStore failed getting subplebbit '${subplebbitAddress}'`,
          )
        );
      }
      ...
      setState((state: any) => ({
        subplebbits: { ...state.subplebbits, [subplebbitAddress]: firstSubplebbitState },
      }));
      ...
      subplebbit.on("update", async (updatedSubplebbit: Subplebbit) => {
        ...
        log("subplebbitsStore subplebbit update", {
          subplebbitAddress,
          updatedSubplebbit,
          account,
        });
        setState((state: any) => ({
          subplebbits: { ...state.subplebbits, [subplebbitAddress]: updatedSubplebbit },
        }));

        // if a subplebbit has a role with an account's address add it to the account.subplebbits
        accountsStore
          .getState()
          .accountsActionsInternal.addSubplebbitRoleToAccountsSubplebbits(updatedSubplebbit);
        ...
        (clientState: string, clientType: string, clientUrl: string, chainTicker?: string) => {
          setState((state: SubplebbitsState) => {
            ...
            return {
              subplebbits: {
                ...state.subplebbits,
                [subplebbitAddress]: { ...state.subplebbits[subplebbitAddress], clients },
              },
            };
          });
        },
      ...
      subplebbit
        .update()
        .catch((error: unknown) => log.trace("subplebbit.update error", { subplebbit, error }));
    },
    ...
    async editSubplebbit(subplebbitAddress: string, subplebbitEditOptions: any, account: Account) {
      assert(
        subplebbitAddress !== "" && typeof subplebbitAddress === "string",
        `subplebbitsStore.editSubplebbit invalid subplebbitAddress argument '${subplebbitAddress}'`,
      );
      assert(
      ...
      assert(
        ...
        `subplebbitsStore.editSubplebbit invalid account argument '${account}'`,
      );

      // if not added to store first, subplebbit.update() is never called
      await getState().addSubplebbitToStore(subplebbitAddress, account);
      ...
      log("subplebbitsStore.editSubplebbit", {
        subplebbitAddress,
    ...
    async createSubplebbit(createSubplebbitOptions: CreateSubplebbitOptions, account: Account) {
      ...
      assert(
        typeof account?.plebbit?.createSubplebbit === "function",
        `subplebbitsStore.createSubplebbit invalid account argument '${account}'`,
      );
      ...
      setState((state: any) => ({
        subplebbits: { ...state.subplebbits, [subplebbit.address]: utils.clone(subplebbit) },
      }));
      return subplebbit;
    },
    ...
    async deleteSubplebbit(subplebbitAddress: string, account: Account) {
      assert(
        subplebbitAddress && typeof subplebbitAddress === "string",
        `subplebbitsStore.deleteSubplebbit invalid subplebbitAddress argument '${subplebbitAddress}'`,
      );
      assert(
        typeof account?.plebbit?.createSubplebbit === "function",
        `subplebbitsStore.deleteSubplebbit invalid account argument '${account}'`,
      );
      ...
      setState((state: any) => ({
        subplebbits: { ...state.subplebbits, [subplebbitAddress]: undefined },
      }));
    },
  }),
);
